...
module.exports = new Promise((resolve, reject) => {
  ...
  portfinder.getPort((err, port) => {
    if (err) {
      ...
      // publish the new Port, necessary for e2e tests// 把获取到的端口号设置为环境变量PORT的值
      process.env.PORT = port
      // add port to devServer config// 重新设置webpack-dev-server的端口的值
      devWebpackConfig.devServer.port = port

      // Add FriendlyErrorsPlugin// 将FriendlyErrorsPlugin添加到webpack的配置文件中
